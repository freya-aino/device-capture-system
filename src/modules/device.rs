...
use cpal::{InputCallbackInfo, StreamConfig, SupportedStreamConfig, SupportedStreamConfigRange};
use tokio;
...
pub struct CpalMicrophoneDevice {
    id: u16,
    name: String,
    microphone_configurations: Vec<SupportedStreamConfigRange>,
...
impl CpalMicrophoneDevice {
    pub fn new(id: u16, cpal_host: &cpal::Host) -> Self {
        ...
        CpalMicrophoneDevice {
            ...
            microphone_configurations: configs,
            microphone: mic,
        }
    }
    ...
    pub fn print_configurations(&self) {
        for (i, config_range) in self.microphone_configurations.iter().enumerate() {
            println!("Configuration {}: {:?}", i, config_range);
        }
    }
    ...
    pub fn open(
        ...
        let config_range = &self.microphone_configurations[selected_config];
        let sr = sample_rate.unwrap_or(16000);
        let config = config_range
            .try_with_sample_rate(cpal::SampleRate(sr))
            .ok_or_else(|| Error::msg(format!("sample rate {} not available", sr)))
            .unwrap()
            .config();
        ...
        let stream = self
            ...
            .build_input_stream(
                &config,
                |data: &[u8], info: &InputCallbackInfo| {
                    // process audio data
